<template>
  <div style="top: 0; left: 0; z-index: 50; position: fixed; width: 100%; height: 100%; pointer-events: none;">
...
onMounted(() => {
  ...
  function hexToRGB(hex: string): ColorRGB {
    let val = hex.replace('#', '');
    if (val.length === 3) val = val[0] as string + val[0] + val[1] + val[1] + val[2] + val[2];
  ...
  function updateFrame() {
    ...
    if (++auto_splashing_count > 180 && auto_splashing_count % 2 == 0) {
      ...
      let wanderX = pointer.texcoordX
      let wanderY = 1 - pointer.texcoordY
      ...
      if (wanderY >= 0.95) { wanderY = 0.95; wanderVY = -Math.abs(wanderVY); }
