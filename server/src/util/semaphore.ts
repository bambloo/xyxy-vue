...
export class Semaphore {
  ...
  consume() {
    return new Promise<void>((resolve, reject) => {
      if (this.closed) {
        return reject(
          new BamblooError(
            BamblooStatusCode.HandleClosed,
            `Semaphore [${this.name}] is closed, Do not consume it anymore`,
          ),
        )
      }
      if (this.resources > 0) {
  ...
  close() {
    ...
    for (var item of this.waiting) {
      item.rejecter(
        new BamblooError(BamblooStatusCode.HandleClosed, `Semaphore [${this.name}] is closed`),
      )
    }
    this.waiting = []
  }
